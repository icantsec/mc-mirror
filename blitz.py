...
def update_monitor():
    ...
    latest_monitor = blitz_img(get_color_map(img))#rle_img(get_color_map(img))
...
def useInput():
    ...
    if inp[0] == "m":
        ...
        pyautogui.keyDown(inp.split("ctrl")[1])
        pyautogui.keyUp('ctrl')
        pyautogui.keyUp(inp.split("ctrl")[1])
        ...
        return jsonify("sent escape")
    elif inp == "center":
        centerX = int(window_tl+(window_wid/2))
        centerY = int(window_tl+(window_height/2))
        pyautogui.moveTo(centerX, centerY)
        return jsonify("centered mouse")
    elif inp == "update":
        title = "League"
        window = gw.getWindowsWithTitle(title)[0]
        window_wid = window.width
        window_height = window.height
        window_tl = window.topleft
        return jsonify("updated window info")
    else:
